---
name: orchestrator
description: Use when the user wants to implement, fix, or refactor something in the pysisense package itself (not a script that uses it) and wants the full structured dev cycle — optional debug/repro, plan, impact analysis, TDD, implementation, validation, CI emulation, and a final report — rather than a single ad-hoc edit. Also use when resuming a cycle that was interrupted or partially run via individual stage skills (`debug`, `plan`, `struct`, `rethink`, `build`, `code`, `validate`, `ci`, `report`). This is the successor to what used to be a flat `dev` checklist skill. For writing scripts that USE pysisense as a library, use the `script` skill instead.
---
