...
## Example 10: Migrate Specific Notebooks

Migrate custom-code notebooks by ID or display name.
...
migration_results = migration.migrate_notebooks(
...
migration_results = migration.migrate_notebooks(
    ...
    action="overwrite",          # Deletes existing notebook on target then recreates from source
)
print(json.dumps(migration_results, indent=4))
```
...
## Example 11: Migrate All Notebooks

Migrate all custom-code notebooks from source to target.

```python
migration_results = migration.migrate_all_notebooks(
    action="skip",               # Options: "skip", "overwrite", "duplicate"
)
print(json.dumps(migration_results, indent=4))
```
