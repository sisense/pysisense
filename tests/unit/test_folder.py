...
class TestGetFolders:
    def test_returns_list_on_success_default_flat(self):
        folder = _make_folder(
            get_responses={"/api/v1/folders": FakeResponse(200, _FOLDERS_FLAT)},
        )
        result = folder.get_folders()
        assert isinstance(result, list)
        assert result[0]["name"] == "Sales"

    def test_passes_custom_structure(self):
        folder = _make_folder(
            get_responses={"/api/v1/folders": FakeResponse(200, _FOLDER_TREE)},
        )
        result = folder.get_folders(structure="tree")
        assert isinstance(result, list)
...
class TestGetFolderAncestors:
    def test_returns_data_on_success(self):
        ancestors = [{"oid": "root", "name": "Root"}]
        folder = _make_folder(
            get_responses={"/api/v1/folders": FakeResponse(200, ancestors)},
        )
        result = folder.get_folder_ancestors("ancestors")
        assert result[0]["name"] == "Root"

    def test_returns_error_on_failure(self):
        folder = _make_folder(
            get_responses={"/api/v1/folders": FakeResponse(404, {"message": "not found"})},
        )
        result = folder.get_folder_ancestors("ancestors")
        assert "error" in result
...
class TestGetNavver:
    def test_returns_navver_dict_on_success(self):
        folder = _make_folder(
            get_responses={"/api/v1/navver": FakeResponse(200, _NAVVER)},
        )
        result = folder.get_navver()
    ...
    def test_returns_error_on_non_200(self):
        folder = _make_folder(
            get_responses={"/api/v1/navver": FakeResponse(403, {"message": "forbidden"})},
        )
        result = folder.get_navver()
        assert "error" in result
