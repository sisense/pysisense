---
name: validate
description: Runs the tests from `build` against the implementation from `code`, and records whether they pass. Runs right after implementation, as part of `orchestrator`'s cycle. Invoke it directly to just re-run tests for a change already in progress.
---
...
A failure in a *different* test file than the one `build` wrote can mean `struct`'s caller-check missed something: it confirmed the production caller needed no change, but didn't check whether that caller's own tests hardcode the old shape. That's a real gap to fix, not a false alarm to dismiss.
