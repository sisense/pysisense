---
name: plan
description: Turns a request to change pysisense's own source into a concrete plan: what's in scope, what kind of change it is, and what "done" looks like. Runs before any impact analysis or code. Normally the first step `orchestrator` takes; invoke it directly when the user just wants a plan written, or is resuming a cycle at this stage.
---
...
- **Definition of done**: an explicit checklist, e.g.:
  ...
  - [ ] TypedDict contract in `pysisense/payloads.py` (only if the change adds/changes a dict-shaped payload parameter)
  - [ ] `@typing_extensions.deprecated` alias kept for the old name (only if this is a rename)
  - [ ] `CHANGELOG.md` and `docs/upgrading.md` entries (only if this is a breaking change)
