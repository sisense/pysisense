---
name: scaffold
description: Sets up a new pysisense automation script as its own small project: folder, `uv init`, a config template, a README, and `.gitignore`. Use this before any script logic is written, whenever the user wants a brand-new script project rather than a quick snippet or an addition to an existing one. Triggers on requests like "set up a new project for a script that does X" or "scaffold a pysisense script". Once the project exists, switch to `/pysisense:script` to write the actual code.
---
...
A pysisense automation script is a deliverable someone else may run, rerun, and hand off. It's not a scratch snippet. Unless the user explicitly asks for a quick inline snippet, or is clearly just pasting into a REPL or an existing project, scaffold it as its own small project:
...
For the script itself, not this plugin's docs. Cover: what the script does, the config file it expects, how to run it (`uv run ...`), and if the script is destructive/bulk, how its dry-run flag works and what to check in the preview before flipping it live.
